...
x01     = [-3.0;1.5;0.0;0;0;0];
x02     = [-3.5;-1.0;0.0;0;0;0];
x03     = [-0.5;0.5;0.0;0;0;0];
...
%% States of mQuad
x   = zeros(6 * N,Ns); x(:,1)   = [x01; x02; x03];
att = zeros(6 * N,Ns); att(:,1) = [att01; att02; att03];
u   = zeros(3 * N,Ns);
u_att = zeros(3 * N,Ns);
thrust = zeros(N,Ns);
roll_d = zeros(N,Ns);
...
for i = 1:1:3
   ...
   if i == 1
       ...
       text(21,0.7,['Collision' newline 'avoidance']);
       text(40,0.2,['Obstacle' newline 'avoidance']);
       rectangle('Position',[12 -0.7 8 1.4],'Curvature',0.2,'EdgeColor','r');
       rectangle('Position',[35 -0.12 20 0.17],'Curvature',0.2,'EdgeColor','r');
       ...
       xlim([12 20]);
       ylim([-0.7 0.7]);
       print(gcf,'Cs2_figure/Cs2_3cf_epx.png','-dpng','-r400'); 
   elseif i == 2
       ylim([-0.8 0.8]);
       ...
       rectangle('Position',[12 -0.5 8 1.1],'Curvature',0.2,'EdgeColor','r');
       ...
       xlim([12 20]);
       ylim([-0.5 0.6]);
       print(gcf,'Cs2_figure/Cs2_3cf_epy.png','-dpng','-r400');
   else
       ylabel('${e_{p,z}}$ (m)','Interpreter','latex','Fontsize',14);
       text(10,-0.35,['Collision' newline 'avoidance']);
       text(42,-0.1,['Obstacle' newline 'avoidance']);
       rectangle('Position',[12 -0.25 8 0.6],'Curvature',0.2,'EdgeColor','r');
       rectangle('Position',[35 -0.04 20 0.1],'Curvature',0.2,'EdgeColor','r');
       rect = [.66 .5 .15 .1];
       ...
       xlim([12 20]);
       ylim([-0.25 0.35]);    
       print(gcf,'Cs2_figure/Cs2_3cf_epz.png','-dpng','-r400');
   end
end
...
%print(gcf,'Cs2_figure/Cs2_3cf_snapshot_FMC.png','-dpng','-r400');

%
figure(5)
set(gcf, 'Position',  [0, 50, 900, 900]);
% 
t_st = [30,36,38,40,42,43,44,45,46,48,50,60]/Ts;
for j = 1:1:12
subplot(4,3,j);
xlim([-2 2]);
ylim([-1.5 1]);
...
print(gcf,'Cs2_figure/Cs2_3cf_snapshot_ob.png','-dpng','-r400');
...
for i = 1:1:N-1
   for j = i:1:N-1
       hold on
       plot(Ts:Ts:(Tsim/Ts)*Ts-Ts,dis_cf((i-1)*(N-1)+j,1:Ns-1),l_pattern(k,:),'linewidth',2);
       %disp((i-1)*(N-1)+j)
       k = k + 1;
   end
end
...
legend(legend_infor);
print(gcf,'Cs2_figure/Cs2_3cf_dis_cf.png','-dpng','-r400');
...
for i = 1:1:N
    hold on
    plot(Ts:Ts:(Tsim/Ts)*Ts-Ts,dis_ob(i,1:Ns-1),l_pattern(i,:),'linewidth',2);
end
grid
%ylim([0 2]);
xlabel('Time (s)');
...
legend(legend_infor);
print(gcf,'Cs2_figure/Cs2_3cf_dis_ob.png','-dpng','-r400');
