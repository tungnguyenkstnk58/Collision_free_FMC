...
att = zeros(6 * N,Ns); 
%att(:,1) = [att01; att02];
%att(:,1) = [att01; att02; att03];
att(:,1) = [att01; att02; att03; att04];
u   = zeros(3 * N,Ns);
u_att = zeros(3 * N,Ns);
thrust = zeros(N,Ns);
roll_d = zeros(N,Ns);
...
for k = 1:1:Ns-1
    ...
    for i = 1:1:N
       fp(1+(i-1)*3:i*3,k) = x(1+(i-1)*6:3+(i-1)*6,1) + (fp_ref(1+(i-1)*3:i*3) - x(1+(i-1)*6:3+(i-1)*6,1)) * step_fcn(k, t_start/Ts, t_end/Ts);
       fp(1+(i-1)*3:i*3,k) = fp(1+(i-1)*3:i*3,k) + [0;0;(i-1)*alt_residue * (1 - step_fcn(k, t_start/Ts, t_end/Ts))];
       fv(1+(i-1)*3:i*3,k) = (fp_ref(1+(i-1)*3:i*3) - x(1+(i-1)*6:3+(i-1)*6,1)) * step_fcn_dot(k, t_start/Ts, t_end/Ts);
    end
...
for i = 1:1:3
   ...
   %hold on;
   for j = 1:1:N
   plot(Ts:Ts:(Tsim/Ts)*Ts-Ts,x(i+(j-1)*6,1:Ns-1)-fp(i+(j-1)*3,1:Ns-1)-r(i,1:Ns-1),'--','linewidth',2);
   %plot(Ts:Ts:(Tsim/Ts)*Ts,fp(i+(j-1)*3,:),'--','linewidth',2);
   hold on;
   ...
   if i == 1
       ...
       box on
       for j = 1:1:N
            plot(12:Ts:20,x(1+(j-1)*6,12/Ts:20/Ts)-fp(i+(j-1)*3,12/Ts:20/Ts)-r(i,12/Ts:20/Ts),'--','linewidth',2);
            hold on;
       end
       grid
       xlim([12 20]);
   ...
   elseif i == 2
       ...
       box on
       for j = 1:1:N
            plot(12:Ts:20,x(2+(j-1)*6,12/Ts:20/Ts)-fp(i+(j-1)*3,12/Ts:20/Ts)-r(i,12/Ts:20/Ts),'--','linewidth',2);
            hold on;
       end
       grid
       xlim([12 20]);
       ...
       box on
       for j = 1:1:N
            plot(12:Ts:20,x(3+(j-1)*6,12/Ts:20/Ts)-fp(i+(j-1)*3,12/Ts:20/Ts)-r(i,12/Ts:20/Ts),'--','linewidth',2);
            hold on;
       end
       grid
       xlim([12 20]);
...
for i = 1:1:N-1
   for j = i:1:N-1
       hold on
       plot(Ts:Ts:(Tsim/Ts)*Ts-Ts,dis_cf((i-1)*(N-1)+j,1:Ns-1),'--','linewidth',2);
       disp((i-1)*(N-1)+j);     
   end
end
...
for i = 1:1:N
   for j = i+1:1:N
       legend_infor = [legend_infor;['Crazyflies no.' num2str(i) ' vs no.' num2str(j)]];
   end
end
grid
%ylim([0 2]);
xlabel('Time (s)');
ylabel('Distance (m)');
legend(legend_infor(2:nchoosek(N,2)+1,:));
...
for i = 1:1:N
    hold on
    plot(Ts:Ts:(Tsim/Ts)*Ts-Ts,dis_ob(i,1:Ns-1),'--','linewidth',2);
    if i > 1
        legend_infor = [legend_infor;['Crazyflie no.' num2str(i) ' vs obstacle']]
    end
end
grid
%ylim([0 2]);
