...
ob_position2 = [-0.9;-1.0;0.3];
ob_position3 = [-2.0;0.8;0.3];
ob_position4 = [0;1.8;0.3];
ob_record    = zeros(4*3,Ns);
...
for k = 1:1:Ns-1
    ...
    %% Obstacle information
    if k == 300
        %annotation('textarrow',[0.75 0.64],[0.7 0.54],'String','Moving','Fontsize',12);
    end
    if k > 250 && k <= 550
        ob_position1 = ob_position1 - [0.1; 0; 0] * Ts;  
    elseif k > 550 && k <= 750
        ob_position1 = ob_position1 + [0.1; 0; 0] * Ts;
    elseif k > 750 && k <= 950
        ob_position2 = ob_position2 + [0; 0.1; 0] * Ts;
    elseif k > 950 && k <= 1050
        ob_position2 = ob_position2 - [0; 0.2; 0] * Ts;
    elseif k > 1050 && k <= 1250
        ob_position3 = ob_position3 + [0.1; 0.0; 0] * Ts;
    elseif k > 1250 && k <= 1350
        ob_position3 = ob_position3 - [0.2; 0.0; 0] * Ts;
    elseif k > 1350 && k <= 1550
        ob_position4 = ob_position4 - [0; 0.1; 0] * Ts;
    elseif k > 1550 && k <= 1650
        ob_position4 = ob_position4 + [0; 0.2; 0] * Ts;
    end    
    ob_position   = [ob_position1,ob_position2,ob_position3,ob_position4];
    ob_record(:,k)= [ob_position1;ob_position2;ob_position3;ob_position4];
...
for i = 1:1:3
   ...
   for j = 1:1:N
        %l_pattern(j)
        plot(Ts:Ts:(Tsim/Ts)*Ts,x(i+(j-1)*6,:)-fp_ref(i+(j-1)*3,:)-r(i,:),l_pattern(j,:),'linewidth',2);
        hold on;
   ...
   if i == 1
       ...
       axes('position',[.31 .6 .26 .3])
       box on
       for j = 1:1:N
            plot(Ts:Ts:tz,x(1+(j-1)*6,1:tz/Ts)-fp_ref(i+(j-1)*3),l_pattern(j,:),'linewidth',2);
            hold on;
       end
       grid
       xlim([0 tz]);
   ...
   elseif i == 2
       ...
       axes('position',[.31 .61 .26 .3])
       box on
       for j = 1:1:N
            plot(Ts:Ts:tz,x(2+(j-1)*6,1:tz/Ts)-fp_ref(i+(j-1)*3),l_pattern(j,:),'linewidth',2);
            hold on;
       end
       grid
       xlim([0 tz]);
...
for i = 1:1:N 
   ...
   text(ob_record(1+(i-1)*3,t_st(j))+0.1,ob_record(2+(i-1)*3,t_st(j))+0.3,['Ob. ' num2str(i)],'Color',[0.8 0.2 0.5]);
end
grid;
...
set(gcf, 'Position',  [0, 50, 900, 900]);
% 
t_st = [56,58,60,62,64,66,68,70,72]/Ts;
for j = 1:1:9
subplot(3,3,j);
xlim([-2 2]);
ylim([-1.8 2.2]);
...
print(gcf,'Cs1_figure/Cs1_snapshot_ob3.png','-dpng','-r400');

%
figure(8)
set(gcf, 'Position',  [0, 50, 900, 900]);
% 
t_st = [74,76,78,80,82,84,86,88,90]/Ts;
for j = 1:1:9
subplot(3,3,j);
xlim([-2 2]);
ylim([-1.8 2.2]);
...
print(gcf,'Cs1_figure/Cs1_snapshot_ob4.png','-dpng','-r400');
